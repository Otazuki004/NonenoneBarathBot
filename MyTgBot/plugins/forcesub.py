...
@bot.on_message(filters.incoming, group=100)
async def ForceSub(_, message):
     ...
     if chat_id in fsub_chats():
          ...
          if not bot_stats.privileges.can_restrict_members:
              ...
              try:
                  xx = await bot.get_chat_member(fsub_channel, user_id)
              except UserNotParticipant:
                    link = (await bot.get_chat(fsub_channel)).invite_link
...
@bot.on_callback_query(filters.regex("fsub_user"))
async def unmute_fsubbed(_, query):
       ...
       if not user_id == query.from_user.id:
           ...
           channel = xx["channel"]
           try:
              hmm = await bot.get_chat_member(chat_id=channel, user_id=user_id)
           except UserNotParticipant:
...
async def ForceSubscribe(_, message):
      ...
      elif not get.privileges.can_change_info: 
      ...
      if message.text.split()[1] == "on":
           ...
           try:
               ...
               hmm = await bot.get_chat_member(chat_id=Fsub_channel, user_id=bot_id)
           except UserNotParticipant:
           ...
           fsub_chat = await bot.get_chat(Fsub_channel)
           x = db.find_one({"chat_id": chat_id})
           if x:
